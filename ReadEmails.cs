...
namespace MSGraphEmailReader
{
    public class ReadEmails
    {
        public async Task<List<GraphMail>> ReadEmailAsync(GraphEmailRequest graphEmailRequest)
        {
            ...
            List<QueryOption> queryOptions = FilterMailsByRequestedDateTime(graphEmailRequest);
        ...
        private List<QueryOption> FilterMailsByRequestedDateTime(GraphEmailRequest graphEmailRequest)
        {
            // Calculate the time difference in hours between the current time and the requested time
            TimeSpan timeDifference = graphEmailRequest.RequestedDateTime - DateTimeOffset.Now;

            // Create the filter condition for ReceivedDateTime using the calculated time difference
            List<QueryOption> queryOptions = new()
            {
                new QueryOption("$filter", $"ReceivedDateTime ge {DateTimeOffset.Now.AddHours(-timeDifference.TotalHours):yyyy-MM-ddTHH:mm:ssZ}")
            };
            return queryOptions;
        }
